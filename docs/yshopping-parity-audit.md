...
> 上述计数是**首轮**对照的结论。本文件持续更新，§5「有意偏离」此后已增补至 **§5.1–§5.19**，
> 最新一条是 2026-08-31 登记的全栈中英文语言切换与内容本地化（§5.19，我方新增能力）。
...
### 5.15 环比/同比对比查询（我方增强，参考项目没有该能力）

参考项目全仓库无对比周期实现：`QuestionIntent.referencesPriorData` 是"分析上文明细"语义，
与环比/同比无关；`PromptLoopAnalysisService` 对指标类回答只做单一区间的合计/最新值/峰值分析，
不取两个可比周期、不算变化百分比。2026-08-22 用户裁定 D3 选方案（b）：作为我方增强实现，
2026-08-23 已交付。

设计边界：`QueryIntent.comparison` 只表达比较意图（`NONE`/`PREVIOUS_PERIOD`/`YEAR_OVER_YEAR`），
不提供第二期日期、公式或百分比；`app.analytics.dates.shift_baseline_period()` 按自然月/自然年
整体平移主周期得到基期（未结束的自然月与上月同期比较，避免"本月至今"对"完整上月"）；
`SafeQueryService` 用两次真实查询取两期数据，后端以 `Decimal` 算出变化率；基期无数据或为零时
不计算比例，显式说明（R7）；对比与维度拆分不能同时请求（两期行数/行序无法一一对应）。

### 5.16 Chat BI 衡量层与管理员看板（我方增强，参考项目没有该能力）

参考项目全仓库没有回复采纳率、用户侧/系统侧准确率、平均思考时长、问题命中率、回答失效率或
运营看板的实现；它只在 `merchant_ai_answer` 保存了可供后续统计的原始回答字段。我方新增
`chatbi_qa_daily` 日粒度汇总和受 `X-Admin-Token` 保护的 `/ops-dashboard`，按日期、商家、问题
分类累加原始计数，并在应用层计算比率。汇总表不存比率，因而口径调整后可幂等重刷历史数据；
分母不足的比率返回 `null` 并在页面展示「样本不足」，不伪装为 0%。这是面向运营质量治理的
增强，不是参考项目还原缺口。

### 5.17 旧 Wiki 的 `ddl` 与 `指标或调用指标平台mcp的skill` 两板块不导入内容

**结构一致，内容裁剪。** `BUSINESS_SECTIONS` 四个板块（`业务流程`/`业务名词解释`/`ddl`/
`指标或调用指标平台mcp的skill`）与参考项目 `WikiPathPolicy.BUSINESS_SECTIONS` 逐字相同；
`_business_root()` 与参考项目 `WikiAdminService` 一样遍历全部四个板块，空板块照样渲染为
空目录节点。差异只在导入的数据集：参考 Wiki 共 43 篇，`wiki_import.py` 的 `_EXCLUDED_DIRS`
排除上述两个板块共 20 篇，实际导入 23 篇。

**理由是数据模型不同，不是遗漏。** 旧 `ddl/*.md` 描述的是 `yshopping.ads_merchant_profile`、
`yshopping.dwm_trade_order_detail_di` 等 Doris 数仓宽表，金额单位为分；旧
`指标或调用指标平台mcp的skill/*.md` 描述的是查指标平台表 `zz.ads_meta_ai_helper_input_metrics`。
我方的经营数据是 `orders`/`order_items`/`refunds`/`products`/`support_tickets` 六张 PostgreSQL
表（金额为元），指标口径来自 `metric_definitions` 与三级检索，两者都不存在。若照原样导入，
知识检索会把描述不存在的表、错误金额单位的文档喂给回答模型，且该错误不触发任何降级分支
（R7 无法覆盖静默错答）——风险高于「树上两个板块为空」。

**2026-08-24 用户裁定：维持 23 篇不变，登记为有意偏离。** 由 `_EXCLUDED_DIRS` 与
`test_parse_wiki_tree_excludes_legacy_ddl_documents` 共同锁定。若将来我方数据模型改为沿用
数仓宽表，本条应重新评估。

### 5.18 顶栏知识库入口改为图标 + 文字标签

参考项目 `App.vue` 的知识库入口是纯图标（`BookOpenText` 配 `title` 属性作为悬停提示），我方
`AssistantView.vue` 此前与之一致。**2026-08-25 用户裁定**：纯图标对新用户不可发现，改为图标
旁加文字「知识库」，与同一顶栏的「新会话」按钮同族（`.knowledge-link` 复用 `.new-chat-button`
的布局与描边风格）。≤560px 断点下与「新会话」一样收起文字、退回 36px 纯图标，避免窄屏顶栏拥挤。
这是纯前端可发现性改动，不涉及后端契约、不影响 `docs/PRD.md` §11 的接口清单。

同批还补齐了知识库维护后台前端此前缺失的**新建文档 / 新建业务域 / 重命名业务域 / 删除节点**
四个操作（后端端点 `backend/app/api/routes/knowledge.py` 早已实现，见 §5.10；此前只有前端读取
和保存两个操作接入，新建、删除、业务域管理均未接入 UI）。这不是新的偏离，是补齐此前的还原缺口。

### 5.19 全栈中英文语言切换与内容本地化（我方增强，参考项目没有该能力）

参考项目全仓库没有界面语言切换、内容翻译缓存或跨语言检索规范化：`yshopping-prototype`、
原 Vue `App.vue` 和 Java 后端均只有中文文案与中文数据，不存在 `SupportedLocale`、
`Accept-Language`/`Content-Language` 协商或任何双语渲染路径。

**2026-08-31 用户明确新增能力，不是还原遗漏。** 完整设计见
`plans/2026-08-31-full-stack-bilingual-localization.md`：第一版支持精确的 `zh-CN`/`en-US`
两种界面语言，新 AI 内容按目标语言直接生成，历史会话、知识正文、日报、看板和 CSV 导出提供
按需翻译的显示副本，原始数据与知识事实源永不因切换语言被覆盖，机器翻译费用计入现有
`LlmCostGuard` 预算并独立报告。产品范围见 `docs/PRD.md` §7.5，验收标准见 §16.1，精确 API
契约见 `docs/backend-development-plan.md` §8.6，前端数据流见
`docs/frontend-development-plan.md` §5.10。

因为这是我方在参考项目之上新增的能力而非还原缺口，本条不登记为 🔴 真实缺口。
